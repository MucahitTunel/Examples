...
export const addPipesAtLocation = (x, world, entities, dispatch) => {
...
const Physics = (entities, { touches, time, dispatch }) => {
    ...
    touches.filter(t => t.type === "press").forEach(t => {
        if (!hadTouches){
            if (world.gravity.y === 0.0){
                ...
                addPipesAtLocation((Constants.MAX_WIDTH * 2) - (Constants.PIPE_WIDTH / 2), world, entities, dispatch)
                addPipesAtLocation((Constants.MAX_WIDTH * 3) - (Constants.PIPE_WIDTH / 2), world, entities, dispatch)
    ...
    //gravity normalde 1 olarak var. Update metodunu çağırınca işleve geçiyor
